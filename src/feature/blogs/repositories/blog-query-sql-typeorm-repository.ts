...
import { ViewBlog, ViewBlogWithUserInfo } from '../api/types/views';
...
export class BlogQuerySqlTypeormRepository {
  ...
  async getBlogsWithUserInfo(queryParamsBlog: QueryParamsInputModel) {
...
*/
    ...
    const arrayBlogs: ViewBlogWithUserInfo[] = result[0].map((blog) => {
      return {
        id: blog.id,
        name: blog.name,
